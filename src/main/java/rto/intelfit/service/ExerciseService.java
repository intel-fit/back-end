...
public class ExerciseService {
    ...
    public ExerciseDto.CalorieResponse recordExerciseAndCalculateCalories(ExerciseDto.CalorieRequest request) {
        ...
        int completedSets = request.getSets();

        // workout_records 갱신
        WorkoutRecord record = workoutRecordRepository.findByUserIdAndWorkoutDate(user.getId(), today)
                .orElse(WorkoutRecord.builder()
                        .user(user)
                        .workoutDate(today)
                        .setsCompleted(0)
                        .build());
        record.setSetsCompleted(record.getSetsCompleted() + completedSets);
        workoutRecordRepository.save(record);

        // 계획 세트 수 조회
        String dayOfWeek = today.getDayOfWeek().name();
        int plannedSets = workoutPlanDetailRepository.countPlannedSetsByDay(user.getId(), dayOfWeek);

        // 운동 달성률 계산
        double rate = (plannedSets == 0) ? 0.0 : ((double) completedSets / plannedSets) * 100.0;
        log.info("✅ 운동 달성률 업데이트 완료: {} / {} 세트 = {}%", completedSets, plannedSets, rate);

        // DailyProgress 반영
        dailyProgressService.calculateTodayProgress(user);
